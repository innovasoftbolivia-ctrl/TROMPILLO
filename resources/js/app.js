...
// Light switcher — se re-vincula tras cada navegación Livewire (wire:navigate),
// porque DOMContentLoaded no se vuelve a disparar y el checkbox se re-renderiza.
function initLightSwitch() {
  document.querySelectorAll('.light-switch').forEach((lightSwitch) => {
    // Sincroniza el estado visual del interruptor con el tema guardado
    lightSwitch.checked = localStorage.getItem('dark-mode') === 'true';
    if (lightSwitch.dataset.lsBound) return; // evita doble binding en el mismo elemento
    lightSwitch.dataset.lsBound = 'true';
    lightSwitch.addEventListener('change', () => {
      const { checked } = lightSwitch;
      document.querySelectorAll('.light-switch').forEach((el) => { el.checked = checked; });
...
document.addEventListener('DOMContentLoaded', initLightSwitch);
document.addEventListener('livewire:navigated', initLightSwitch);
