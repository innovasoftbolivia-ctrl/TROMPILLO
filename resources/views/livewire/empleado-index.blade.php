<div>
    @php $cargos = ['piloto' => 'Piloto', 'copiloto' => 'Copiloto', 'tecnico' => 'Técnico', 'despachador' => 'Despachador', 'administrativo' => 'Administrativo', 'ventas' => 'Ventas', 'gerente' => 'Gerente']; @endphp <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            <div class="mb-4 sm:mb-0">
                <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-200 font-bold">Empleados 👷</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Gestión del personal de la aerolínea.</p>
            </div> <a href="{{ route('empleados.create') }}"
                ...
                </svg><span class="max-xs:sr-only ml-2">Nuevo empleado</span></a>
        </div>
        @if ($flashOk)
                ...
                <div class="bg-white dark:bg-slate-800 shadow-xs rounded-xl p-4 mb-6">
                    <div class="grid grid-cols-12 gap-4 items-end">
                        <div class="col-span-12 sm:col-span-6"><label
                                class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Buscar</label><input
                                wire:model.live.debounce.400ms="buscar" type="text" class="form-input w-full"
                                placeholder="Nombre, apellido o documento"></div>
                        <div class="col-span-6 sm:col-span-4"><label
                                class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Cargo</label><select
                                wire:model.live="cargo" class="form-select w-full">
                ...
                <div class="bg-white dark:bg-slate-800 shadow-xs rounded-xl">
                    <div class="px-5 py-4 border-b border-gray-100 dark:border-slate-700 /60">
                        <h2 class="font-semibold text-gray-800 dark:text-gray-200 ">Listado <span
                                class="text-gray-400 dark:text-gray-500 font-medium">({{ $empleados->total() }})</span>
                        </h2>
                    </div>
                        ...
                        <table class="table-auto w-full dark:text-gray-300">
                            <thead
                                ...
                                <tr>
                                    ...
                                    <th class="px-4 py-3 whitespace-nowrap text-left">Cargo</th>
                            ...
                            <tbody class="text-sm divide-y divide-gray-100 dark:divide-slate-700 /60">
                                @forelse ($empleados as $e)
                                    <tr wire:key="empleado-{{ $e->id }}">
                                        <td class="px-4 py-3 whitespace-nowrap">
                                            <div class="font-medium text-gray-800 dark:text-gray-200 ">
                                                {{ $e->nombres }} {{ $e->apellidos }}</div>
                                        </td>
                                        <td class="px-4 py-3 whitespace-nowrap">
                                            <div class="text-gray-800 dark:text-gray-200 ">{{ $e->numero_documento }}
                                            </div>
                                            <div class="text-xs text-gray-500">{{ $e->tipo_documento }}</div>
                                        </td>
                                        <td class="px-4 py-3 whitespace-nowrap"><span
                                                class="inline-flex text-xs font-medium rounded-full px-2.5 py-1 bg-sky-500/15 text-sky-600">{{ $cargos[$e->cargo] ?? $e->cargo }}</span>
                                        </td>
                                        <td class="px-4 py-3 whitespace-nowrap text-center"><span
                                                class="inline-flex text-xs font-medium rounded-full px-2.5 py-1 {{ $e->activo ? 'bg-emerald-500/15 text-emerald-600' : 'bg-gray-500/20 text-gray-600' }}">{{ $e->activo ? 'Sí' : 'No' }}</span>
                                        </td>
                                        <td class="px-4 py-3 whitespace-nowrap text-right">
                                            <div class="flex items-center justify-end gap-2"> <a
                                                    href="{{ route('empleados.show', $e) }}"
                                                    ...
                                                    </svg></a> <a href="{{ route('empleados.edit', $e) }}"
                                                    ...
                                                    </svg></a> <button wire:click="eliminar({{ $e->id }})"
                                                    wire:confirm="¿Eliminar a {{ $e->nombres }} {{ $e->apellidos }}?"
                                        ...
                                        <td colspan="5"
                                            ...
                                            empleados. <a href="{{ route('empleados.create') }}"
