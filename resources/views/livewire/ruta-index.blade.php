    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            <div class="mb-4 sm:mb-0">
                <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-200 font-bold">Rutas 🗺️</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Gestión de rutas entre aeropuertos.</p>
            </div> <a href="{{ route('rutas.create') }}" class="btn bg-emerald-500 hover:bg-emerald-600 text-white"> <svg
                    class="fill-current shrink-0 xs:hidden" width="16" height="16" viewBox="0 0 16 16">
                    <path
                        d="M15 7H9V1c0-.6-.4-1-1-1S7 .4 7 1v6H1c-.6 0-1 .4-1 1s.4 1 1 1h6v6c0 .6.4 1 1 1s1-.4 1-1V9h6c.6 0 1-.4 1-1s-.4-1-1-1z" />
                </svg> <span class="max-xs:sr-only ml-2">Nueva ruta</span> </a>
        </div>
        @if ($flashOk)
                ...
                @endif @if (session('success'))
                    ...
                    <div class="bg-white dark:bg-slate-800 shadow-xs rounded-xl p-4 mb-6">
                        <div class="grid grid-cols-12 gap-4 items-end">
                            <div class="col-span-12 sm:col-span-7"> <label
                                ...
                                <input wire:model.live.debounce.400ms="buscar" type="text" class="form-input w-full"
                                    placeholder="Ciudad de origen o destino"> </div>
                            <div class="col-span-6 sm:col-span-3"> <label
                                    class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Activa</label>
                                <select wire:model.live="activa" class="form-select w-full">
                    ...
                    <div class="bg-white dark:bg-slate-800 shadow-xs rounded-xl">
                        <div class="px-5 py-4 border-b border-gray-100 dark:border-slate-700 /60">
                            <h2 class="font-semibold text-gray-800 dark:text-gray-200 ">Listado <span
                                    class="text-gray-400 dark:text-gray-500 font-medium">({{ $rutas->total() }})</span>
                            </h2>
                        </div>
                        <div class="overflow-x-auto">
                            <table class="table-auto w-full dark:text-gray-300">
                                <thead
                                    ...
                                    <tr>
                                        <th class="px-4 py-3 whitespace-nowrap text-left">Ruta</th>
                                        <th class="px-4 py-3 whitespace-nowrap text-right">Distancia (km)</th>
                                        <th class="px-4 py-3 whitespace-nowrap text-right">Duración (min)</th>
                                        <th class="px-4 py-3 whitespace-nowrap text-right">Precio base</th>
                                        <th class="px-4 py-3 whitespace-nowrap text-center">Activa</th>
                                ...
                                <tbody class="text-sm divide-y divide-gray-100 dark:divide-slate-700 /60">
                                    @forelse ($rutas as $ruta)
                                        <tr wire:key="ruta-{{ $ruta->id }}">
                                            <td class="px-4 py-3 whitespace-nowrap">
                                                <div class="flex items-center"> <span
                                                        class="font-medium text-gray-700 dark:text-gray-200">{{ $ruta->origen->ciudad ?? '?' }}</span>
                                                    <svg class="w-4 h-4 mx-1.5 text-gray-400 fill-current"
                                                        viewBox="0 0 16 16">
                                                        <path d="M11.4 8L8 4.6 9.4 3.2 15.2 8 9.4 12.8 8 11.4z" />
                                                        <path d="M0 7h13v2H0z" />
                                                    </svg> <span
                                                        class="font-medium text-gray-700 dark:text-gray-200">{{ $ruta->destino->ciudad ?? '?' }}</span>
                                                </div>
                                            </td>
                                            <td
                                                class="px-4 py-3 whitespace-nowrap text-right text-gray-800 dark:text-gray-200 ">
                                                {{ $ruta->distancia_km !== null ? number_format($ruta->distancia_km, 0, ',', '.') : '—' }}
                                            </td>
                                            <td
                                                class="px-4 py-3 whitespace-nowrap text-right text-gray-800 dark:text-gray-200 ">
                                                {{ $ruta->duracion_estimada_min !== null ? number_format($ruta->duracion_estimada_min, 0, ',', '.') : '—' }}
                                            </td>
                                            <td
                                                class="px-4 py-3 whitespace-nowrap text-right font-medium text-gray-800 dark:text-gray-200 ">
                                                Bs {{ number_format($ruta->precio_base, 0, ',', '.') }}</td>
                                            <td class="px-4 py-3 whitespace-nowrap text-center"> <span
                                                    class="inline-flex text-xs font-medium rounded-full text-center px-2.5 py-1 {{ $ruta->activa ? 'bg-emerald-500/15 text-emerald-600 dark:text-emerald-400' : 'bg-gray-500/20 text-gray-600 dark:text-gray-400' }}">{{ $ruta->activa ? 'Sí' : 'No' }}</span>
                                            </td>
                                            <td class="px-4 py-3 whitespace-nowrap text-right">
                                                <div class="flex items-center justify-end gap-2"> <a
                                                        ...
                                                        </svg></a> <a href="{{ route('rutas.edit', $ruta) }}"
                                                        ...
                                                        </svg></a> <button wire:click="eliminar({{ $ruta->id }})"
                                                        wire:confirm="¿Eliminar la ruta {{ $ruta->origen->ciudad ?? '?' }} → {{ $ruta->destino->ciudad ?? '?' }}?"
                                            ...
                                            <td colspan="6"
                                                ...
                                                rutas que coincidan. <a href="{{ route('rutas.create') }}"
                                                    class="text-emerald-500 hover:text-emerald-600 font-medium">Crea la
                                                    primera</a>.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
