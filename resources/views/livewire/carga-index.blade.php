<div>
    @php $estadosCss = ['registrado'=>['label'=>'Recibido','css'=>'bg-sky-500/15 text-sky-600'],'en_transito'=>['label'=>'En tránsito','css'=>'bg-amber-500/15 text-amber-600'],'entregado'=>['label'=>'Entregado','css'=>'bg-emerald-500/15 text-emerald-600'],'devuelto'=>['label'=>'Devuelto','css'=>'bg-red-500/15 text-red-600']]; @endphp <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            <div class="mb-4 sm:mb-0">
                <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-200 font-bold">Envíos de Carga 📦</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Gestión de envíos de carga aérea.</p>
            </div> <a href="{{ route('carga.create') }}" class="btn bg-emerald-500 hover:bg-emerald-600 text-white"><svg
                    class="fill-current shrink-0 xs:hidden" width="16" height="16" viewBox="0 0 16 16">
                    <path
                        d="M15 7H9V1c0-.6-.4-1-1-1S7 .4 7 1v6H1c-.6 0-1 .4-1 1s.4 1 1 1h6v6c0 .6.4 1 1 1s1-.4 1-1V9h6c.6 0 1-.4 1-1s-.4-1-1-1z" />
                </svg><span class="max-xs:sr-only ml-2">Nuevo envío</span></a>
        </div>
        @if ($flashOk)
            ...
            @endif @if ($flashError)
                ...
                <div class="bg-white dark:bg-slate-800 shadow-xs rounded-xl p-4 mb-6">
                    <div class="grid grid-cols-12 gap-4 items-end">
                        <div class="col-span-12 sm:col-span-6"><label
                                class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Buscar
                                (guía)</label><input wire:model.live.debounce.400ms="buscar" type="text"
                                class="form-input w-full" placeholder="Número de guía"></div>
                ...
                <div class="bg-white dark:bg-slate-800 shadow-xs rounded-xl">
                    <div class="px-5 py-4 border-b border-gray-100 dark:border-slate-700 /60">
                        <h2 class="font-semibold text-gray-800 dark:text-gray-200 ">Listado <span
                                class="text-gray-400 dark:text-gray-500 font-medium">({{ $envios->total() }})</span>
                        </h2>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="table-auto w-full dark:text-gray-300">
                            ...
                            <tbody class="text-sm divide-y divide-gray-100 dark:divide-slate-700 /60">
                                @forelse ($envios as $e)
                                    <tr wire:key="envio-{{ $e->id }}">
                                        <td
                                            class="px-4 py-3 whitespace-nowrap font-medium text-gray-800 dark:text-gray-200 ">
                                            {{ $e->guia }}</td>
                                        <td class="px-4 py-3 whitespace-nowrap">{{ $e->remitente }}</td>
                                        <td class="px-4 py-3 whitespace-nowrap">{{ $e->destinatario }}</td>
                                        <td class="px-4 py-3 whitespace-nowrap text-right">
                                            {{ number_format($e->peso_kg, 1, ',', '.') }}</td>
                                        <td class="px-4 py-3 whitespace-nowrap text-right font-medium">Bs
                                            {{ number_format($e->costo_envio, 0, ',', '.') }}</td>
                                        <td class="px-4 py-3 whitespace-nowrap text-center"><span
                                                class="inline-flex text-xs font-medium rounded-full px-2.5 py-1 {{ $estadosCss[$e->estado]['css'] ?? 'bg-gray-500/20 text-gray-600' }}">{{ $estadosCss[$e->estado]['label'] ?? $e->estado }}</span>
                                        </td>
                                        <td class="px-4 py-3 whitespace-nowrap text-right">
                                            <div class="flex items-center justify-end gap-2"> <a
                                                    href="{{ route('carga.show', $e) }}"
                                                    ...
                                                    </svg></a> <a href="{{ route('carga.edit', $e) }}"
                                                    class="text-gray-400 hover:text-emerald-500"><svg
                                                        class="w-5 h-5 fill-current" viewBox="0 0 20 20">
                                                        <path
                                                    ...
                                                    wire:confirm="¿Eliminar el envío {{ $e->guia }}?"
                                        ...
                                        <td colspan="7"
                                            ...
                                            envíos. <a href="{{ route('carga.create') }}"
                ...
                <div class="mt-6">{{ $envios->links() }}</div>
    </div>
</div>
