    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            <div class="mb-4 sm:mb-0">
                <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-200 font-bold">Personas 👤</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Clientes, proveedores y contactos (natural y
                    jurídica).</p>
            </div> <a href="{{ route('personas.create') }}" class="btn bg-emerald-500 hover:bg-emerald-600 text-white">
                <svg class="fill-current shrink-0 xs:hidden" width="16" height="16" viewBox="0 0 16 16">
                    <path
                        d="M15 7H9V1c0-.6-.4-1-1-1S7 .4 7 1v6H1c-.6 0-1 .4-1 1s.4 1 1 1h6v6c0 .6.4 1 1 1s1-.4 1-1V9h6c.6 0 1-.4 1-1s-.4-1-1-1z" />
                </svg> <span class="max-xs:sr-only ml-2">Nueva persona</span> </a>
        </div>
        @if ($flashOk)
            ...
            @endif @if ($flashError)
                ...
                @endif @if (session('success'))
                        ...
                        <div class="grid grid-cols-12 gap-4 items-end">
                            <div class="col-span-12 sm:col-span-7"> <label
                                ...
                                <input wire:model.live.debounce.400ms="buscar" type="text" class="form-input w-full"
                                    placeholder="Documento, nombre o razón social"> </div>
                            <div class="col-span-6 sm:col-span-3"> <label
                                    class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Tipo</label>
                                <select wire:model.live="tipo_persona" class="form-select w-full">
                                    <option value="">Todos</option>
                                    <option value="natural">Natural</option>
                                    <option value="juridica">Jurídica</option>
                                </select> </div>
                            <div class="col-span-6 sm:col-span-2 flex gap-2">
                                @if ($buscar !== '' || $tipo_persona !== '')
                    ...
                    <div class="bg-white dark:bg-slate-800 shadow-xs rounded-xl">
                        <div class="px-5 py-4 border-b border-gray-100 dark:border-slate-700 /60">
                            <h2 class="font-semibold text-gray-800 dark:text-gray-200 ">Listado <span
                                    class="text-gray-400 dark:text-gray-500 font-medium">({{ $personas->total() }})</span>
                            </h2>
                        </div>
                        <div class="overflow-x-auto">
                            <table class="table-auto w-full dark:text-gray-300">
                                <thead
                                    ...
                                    <tr>
                                        <th class="px-4 py-3 whitespace-nowrap text-left">Nombre / Razón social</th>
                                        <th class="px-4 py-3 whitespace-nowrap text-left">Documento</th>
                                        <th class="px-4 py-3 whitespace-nowrap text-left">Tipo</th>
                                        <th class="px-4 py-3 whitespace-nowrap text-left">Contacto</th>
                                        ...
                                        <tr wire:key="persona-{{ $p->id }}">
                                            <td class="px-4 py-3 whitespace-nowrap">
                                                <div class="font-medium text-gray-800 dark:text-gray-200 ">
                                                    {{ $p->nombre_completo }}</div>
                                            </td>
                                            <td class="px-4 py-3 whitespace-nowrap">
                                                <div class="text-gray-800 dark:text-gray-200 ">
                                                    {{ $p->numero_documento }}</div>
                                                <div class="text-xs text-gray-500 dark:text-gray-400">
                                                    {{ $p->tipo_documento }}</div>
                                            </td>
                                            <td class="px-4 py-3 whitespace-nowrap"> <span
                                                    class="inline-flex text-xs font-medium rounded-full px-2.5 py-1 {{ $p->tipo_persona === 'natural' ? 'bg-sky-500/15 text-sky-600' : 'bg-violet-500/15 text-violet-600' }}">{{ ucfirst($p->tipo_persona) }}</span>
                                            </td>
                                            <td class="px-4 py-3 whitespace-nowrap">
                                                <div>{{ $p->telefono ?? '—' }}</div>
                                            ...
                                            <td class="px-4 py-3 whitespace-nowrap text-right">
                                                <div class="flex items-center justify-end gap-2"> <a
                                                        ...
                                                        </svg></a> <a href="{{ route('personas.edit', $p) }}"
                                                        ...
                                                        wire:confirm="¿Eliminar a {{ $p->nombre_completo }}?"
                                            ...
                                            <td colspan="5"
                                                ...
                                                personas que coincidan. <a href="{{ route('personas.create') }}"
                    ...
                    <div class="mt-6">{{ $personas->links() }}</div>
    </div>
</div>
