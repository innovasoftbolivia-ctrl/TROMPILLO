<div> @php $estados = ['activa' => ['label' => 'Activa', 'css' => 'bg-emerald-500/15 text-emerald-600'], 'mantenimiento' => ['label' => 'En mantenimiento', 'css' => 'bg-amber-500/15 text-amber-600'], 'inactiva' => ['label' => 'Inactiva', 'css' => 'bg-gray-500/20 text-gray-600'], 'retirada' => ['label' => 'Retirada', 'css' => 'bg-gray-500/20 text-gray-600']]; @endphp <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            <div class="mb-4 sm:mb-0">
                <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-200 font-bold">Aeronaves ✈️</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Gestión de la flota de aeronaves.</p>
            </div> <a href="{{ route('aeronaves.create') }}" class="btn bg-emerald-500 hover:bg-emerald-600 text-white">
                <svg class="fill-current shrink-0 xs:hidden" width="16" height="16" viewBox="0 0 16 16">
                    <path
                        d="M15 7H9V1c0-.6-.4-1-1-1S7 .4 7 1v6H1c-.6 0-1 .4-1 1s.4 1 1 1h6v6c0 .6.4 1 1 1s1-.4 1-1V9h6c.6 0 1-.4 1-1s-.4-1-1-1z" />
                </svg> <span class="max-xs:sr-only ml-2">Nueva aeronave</span> </a>
        </div>
        @if ($flashOk)
            ...
            @endif @if ($flashError)
                ...
                @endif @if (session('success'))
                        ...
                        <div class="grid grid-cols-12 gap-4 items-end">
                            <div class="col-span-12 sm:col-span-6"> <label
                                ...
                                <input wire:model.live.debounce.400ms="buscar" type="text" class="form-input w-full"
                                    placeholder="Matrícula, modelo o fabricante"> </div>
                            <div class="col-span-6 sm:col-span-4"> <label
                                    class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Estado</label>
                    ...
                    <div class="bg-white dark:bg-slate-800 shadow-xs rounded-xl">
                        <div class="px-5 py-4 border-b border-gray-100 dark:border-slate-700 /60">
                            <h2 class="font-semibold text-gray-800 dark:text-gray-200 ">Listado <span
                                    class="text-gray-400 dark:text-gray-500 font-medium">({{ $aeronaves->total() }})</span>
                            </h2>
                        </div>
                        <div class="overflow-x-auto">
                            <table class="table-auto w-full dark:text-gray-300">
                                <thead
                                    ...
                                    <tr>
                                        <th class="px-4 py-3 whitespace-nowrap text-left">Matrícula</th>
                                        <th class="px-4 py-3 whitespace-nowrap text-left">Modelo</th>
                                        <th class="px-4 py-3 whitespace-nowrap text-right">Capacidad</th>
                                ...
                                <tbody class="text-sm divide-y divide-gray-100 dark:divide-slate-700 /60">
                                    @forelse ($aeronaves as $a)
                                        <tr wire:key="aeronave-{{ $a->id }}">
                                            <td class="px-4 py-3 whitespace-nowrap">
                                                <div class="font-medium text-gray-800 dark:text-gray-200 ">
                                                    {{ $a->matricula }}</div>
                                            </td>
                                            <td class="px-4 py-3 whitespace-nowrap">
                                                <div class="text-gray-800 dark:text-gray-200 ">{{ $a->modelo }}</div>
                                                <div class="text-xs text-gray-500 dark:text-gray-400">
                                                    {{ $a->fabricante }}</div>
                                            </td>
                                            <td
                                                class="px-4 py-3 whitespace-nowrap text-right text-gray-800 dark:text-gray-200 ">
                                                {{ $a->capacidad_pasajeros ?? '—' }} pax</td>
                                            <td
                                                class="px-4 py-3 whitespace-nowrap text-right text-gray-800 dark:text-gray-200 ">
                                                {{ number_format($a->horas_vuelo_totales ?? 0, 0, ',', '.') }}</td>
                                            <td class="px-4 py-3 whitespace-nowrap text-center"> <span
                                                    class="inline-flex text-xs font-medium rounded-full px-2.5 py-1 {{ $estados[$a->estado]['css'] ?? 'bg-gray-500/20 text-gray-600' }}">{{ $estados[$a->estado]['label'] ?? $a->estado }}</span>
                                            </td>
                                            <td class="px-4 py-3 whitespace-nowrap text-right">
                                                <div class="flex items-center justify-end gap-2"> <a
                                                        href="{{ route('aeronaves.show', $a) }}"
                                                        ...
                                                        </svg></a> <a href="{{ route('aeronaves.edit', $a) }}"
                                                        ...
                                                        </svg></a> <button wire:click="eliminar({{ $a->id }})"
                                                        wire:confirm="¿Eliminar la aeronave {{ $a->matricula }}?"
                                            ...
                                            <td colspan="6"
                                                ...
                                                aeronaves que coincidan. <a href="{{ route('aeronaves.create') }}"
                    ...
                    <div class="mt-6">{{ $aeronaves->links() }}</div>
    </div>
</div>
