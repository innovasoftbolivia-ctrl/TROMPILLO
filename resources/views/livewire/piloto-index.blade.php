<div>
    @php $tipos = ['PPL' => 'PPL - Privado', 'CPL' => 'CPL - Comercial', 'ATPL' => 'ATPL - Transporte', 'PCA' => 'PCA - Aviación Comercial']; @endphp <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            <div class="mb-4 sm:mb-0">
                <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-200 font-bold">Pilotos 🧑‍✈️</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Licencias, horas de vuelo y certificaciones.</p>
            </div> <a href="{{ route('pilotos.create') }}" class="btn bg-emerald-500 hover:bg-emerald-600 text-white"><svg
                    class="fill-current shrink-0 xs:hidden" width="16" height="16" viewBox="0 0 16 16">
                    <path
                        d="M15 7H9V1c0-.6-.4-1-1-1S7 .4 7 1v6H1c-.6 0-1 .4-1 1s.4 1 1 1h6v6c0 .6.4 1 1 1s1-.4 1-1V9h6c.6 0 1-.4 1-1s-.4-1-1-1z" />
                </svg><span class="max-xs:sr-only ml-2">Nuevo piloto</span></a>
        </div>
        @if ($flashOk)
            ...
            @endif @if ($flashError)
                ...
                <div class="bg-white dark:bg-slate-800 shadow-xs rounded-xl p-4 mb-6">
                    <div class="grid grid-cols-12 gap-4 items-end">
                        <div class="col-span-12 sm:col-span-6"><label
                                class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Buscar (nro.
                                licencia)</label><input wire:model.live.debounce.400ms="buscar" type="text"
                                class="form-input w-full" placeholder="Número de licencia"></div>
                        <div class="col-span-6 sm:col-span-4"><label
                                class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Tipo
                                licencia</label><select wire:model.live="tipo_licencia" class="form-select w-full">
                ...
                <div class="bg-white dark:bg-slate-800 shadow-xs rounded-xl">
                    <div class="px-5 py-4 border-b border-gray-100 dark:border-slate-700 /60">
                        <h2 class="font-semibold text-gray-800 dark:text-gray-200 ">Listado <span
                                class="text-gray-400 dark:text-gray-500 font-medium">({{ $pilotos->total() }})</span>
                        </h2>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="table-auto w-full dark:text-gray-300">
                            <thead
                                ...
                                <tr>
                                    <th class="px-4 py-3 whitespace-nowrap text-left">Empleado</th>
                                    <th class="px-4 py-3 whitespace-nowrap text-left">Licencia</th>
                                    <th class="px-4 py-3 whitespace-nowrap text-left">Tipo</th>
                                    <th class="px-4 py-3 whitespace-nowrap text-right">Horas vuelo</th>
                                    <th class="px-4 py-3 whitespace-nowrap text-left">Vencimiento lic.</th>
                            ...
                            <tbody class="text-sm divide-y divide-gray-100 dark:divide-slate-700 /60">
                                @forelse ($pilotos as $p)
                                    <tr wire:key="piloto-{{ $p->id }}">
                                        <td
                                            class="px-4 py-3 whitespace-nowrap font-medium text-gray-800 dark:text-gray-200 ">
                                            {{ $p->empleado ? $p->empleado->nombres . ' ' . $p->empleado->apellidos : '—' }}
                                        </td>
                                        <td class="px-4 py-3 whitespace-nowrap">{{ $p->licencia_numero }}</td>
                                        <td class="px-4 py-3 whitespace-nowrap"><span
                                                class="inline-flex text-xs font-medium rounded-full px-2.5 py-1 bg-violet-500/15 text-violet-600">{{ $p->tipo_licencia }}</span>
                                        </td>
                                        <td class="px-4 py-3 whitespace-nowrap text-right">
                                            {{ number_format($p->horas_vuelo ?? 0, 0, ',', '.') }}</td>
                                        <td class="px-4 py-3 whitespace-nowrap">{{ $p->vencimiento_licencia ?? '—' }}
                                        </td>
                                        <td class="px-4 py-3 whitespace-nowrap text-right">
                                            <div class="flex items-center justify-end gap-2"> <a
                                                    href="{{ route('pilotos.show', $p) }}"
                                                    ...
                                                    </svg></a> <a href="{{ route('pilotos.edit', $p) }}"
                                                    ...
                                                    </svg></a> <button wire:click="eliminar({{ $p->id }})"
                                                    wire:confirm="¿Eliminar a este piloto?"
                                        ...
                                        <td colspan="6"
                                            ...
                                            pilotos. <a href="{{ route('pilotos.create') }}"
                ...
                <div class="mt-6">{{ $pilotos->links() }}</div>
    </div>
</div>
